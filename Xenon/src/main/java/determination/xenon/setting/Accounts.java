...
import determination.xenon.auth.authlibinjector.*;
import determination.xenon.auth.microsoft.MicrosoftAccount;
import determination.xenon.auth.microsoft.MicrosoftAccountFactory;
import determination.xenon.auth.microsoft.MicrosoftService;
import determination.xenon.auth.offline.OfflineAccount;
import determination.xenon.auth.offline.OfflineAccountFactory;
import determination.xenon.auth.yggdrasil.RemoteAuthenticationException;
import determination.xenon.game.OAuthServer;
...
import static determination.xenon.ui.FXUtils.onInvalidating;
import static determination.xenon.util.Lang.immutableListOf;
import static determination.xenon.util.Lang.mapOf;
import static determination.xenon.util.Pair.pair;
import static determination.xenon.util.gson.JsonUtils.listTypeOf;
import static determination.xenon.util.gson.JsonUtils.mapTypeOf;
...
public final class Accounts {
    ...
    private static void loadGlobalAccountStorages() {
        Path globalAccountsFile = Metadata.XENON_GLOBAL_DIRECTORY.resolve("accounts.json");
    ...
    private static AuthlibInjectorArtifactProvider createAuthlibInjectorArtifactProvider() {
        ...
        if (authlibInjectorVersion == null) {
            // Xenon does not bundle authlib-injector (Mindustry has no auth system).
            // Return a stub that resolves to a non-existent jar; account/login UI is hidden.
            return new SimpleAuthlibInjectorArtifactProvider(
                    Metadata.DEPENDENCIES_DIRECTORY.resolve("universal").resolve("authlib-injector-stub.jar"));
        }
