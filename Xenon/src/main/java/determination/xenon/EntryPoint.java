...
public final class EntryPoint {
    ...
    private static void createHMCLDirectories() {
        if (!Files.isDirectory(Metadata.XENON_CURRENT_DIRECTORY)) {
            try {
                Files.createDirectories(Metadata.XENON_CURRENT_DIRECTORY);
                if (OperatingSystem.CURRENT_OS == OperatingSystem.WINDOWS) {
                    try {
                        Files.setAttribute(Metadata.XENON_CURRENT_DIRECTORY, "dos:hidden", true);
                    } catch (IOException e) {
                        LOG.warning("Failed to set hidden attribute of " + Metadata.XENON_CURRENT_DIRECTORY, e);
                    }
                }
            } catch (IOException e) {
                // Logger has not been started yet, so print directly to System.err
                System.err.println("Failed to create HMCL directory: " + Metadata.XENON_CURRENT_DIRECTORY);
                e.printStackTrace(System.err);
                showErrorAndExit(i18n("fatal.create_hmcl_current_directory_failure", Metadata.XENON_CURRENT_DIRECTORY));
            }
        }

        if (!Files.isDirectory(Metadata.XENON_GLOBAL_DIRECTORY)) {
            try {
                Files.createDirectories(Metadata.XENON_GLOBAL_DIRECTORY);
            } catch (IOException e) {
                LOG.warning("Failed to create HMCL global directory " + Metadata.XENON_GLOBAL_DIRECTORY, e);
            }
        }
    }
