...
import determination.xenon.game.JavaVersionConstraint;
...
import determination.xenon.util.platform.windows.WinReg;
...
public final class JavaManager {
    ...
    public static final HMCLJavaRepository REPOSITORY = new HMCLJavaRepository(Metadata.XENON_GLOBAL_DIRECTORY.resolve("java"));
    public static final HMCLJavaRepository LOCAL_REPOSITORY = new HMCLJavaRepository(Metadata.XENON_CURRENT_DIRECTORY.resolve("java"));
    ...
    private static Map<Path, JavaRuntime> searchPotentialJavaExecutables(boolean useCache) {
        Searcher searcher = new Searcher(Metadata.XENON_GLOBAL_DIRECTORY.resolve("javaCache.json"));
        if (useCache)
            searcher.loadCache();
