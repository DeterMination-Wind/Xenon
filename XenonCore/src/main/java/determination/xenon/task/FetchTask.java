...
public abstract class FetchTask<T> extends Task<T> {
    ...
    /**
     * Public hook for non-FetchTask download paths (e.g. the Mindustry
     * {@code MirrorDownloader}) to feed bytes into the global speed
     * counter so the top-left "current downloads" popup shows real
     * throughput instead of 0 B/s.
     *
     * @param bytes positive byte delta written since the last call
     */
