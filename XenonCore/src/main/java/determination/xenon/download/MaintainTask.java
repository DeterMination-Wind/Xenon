...
import determination.xenon.util.SimpleMultimap;
...
public class MaintainTask extends Task<Version> {
    ...
    private static Version maintainGameWithCpwModLauncher(GameRepository repository, Version version) {
        ...
        if (libraryAnalyzer.has(OPTIFINE)) {
            Library hmclTransformerDiscoveryService = new Library(new Artifact("determination.xenon", "transformer-discovery-service", "1.0"));
            Optional<Library> optiFine = version.getLibraries().stream().filter(library -> library.is("optifine", "OptiFine")).findAny();
            boolean libraryExisting = version.getLibraries().stream().anyMatch(library -> library.is("determination.xenon", "transformer-discovery-service"));
