...
/**
 * Converts canonical GitHub release asset URLs into the play.mindustry.men cache URL used
...
public final class HighStarModCache {
    private static final String CACHE_PREFIX = "http://play.mindustry.men/";
    private static final Pattern GITHUB_RELEASE_ASSET = Pattern.compile(
            "^https?://github\\.com/([^/]+)/([^/]+)/releases/download/([^/]+)/([^?#]+)$");

    private HighStarModCache() {}

    /// Returns whether a mod with the given star count should try the play.mindustry.men cache before the normal downloader.
    ...
    /// Rewrites a canonical GitHub release asset URL into the public play.mindustry.men cache URL, or returns `null` when unsupported.
